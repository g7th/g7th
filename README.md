...
# `if` <stuff to do> `endif` <rest of program>.
...
# Loop with `begin` <flag> `while` <stuff to do> `endwhile`:
: count_down 10 begin 1- dup 0>= while dup . space endwhile ;

# Indefinite loop with `repeat` <stuff to do> <flag> `until` if <flag> remains false:
: death repeat ." Are we there yet?" cr false until ;

# Phonetic alphabet with the `case` <match> `of` <stuff to do> `endof` `endcase`
# construct:
'G'
case
  'A' of ."Alpha" cr
...
8.3e1 0.8e1 +             (*9.1*)
7 9 /                     (*0.77777778*)
# If you want to make a integer division:
7 9 //                    (*0*)
7 9 /mod                  (*0 7*)
...
# Loading `g7th` files:
# load load.7th                 (*eg. load.7th is loaded once on program start*)
